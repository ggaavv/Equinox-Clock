...
void TCPTXPerformanceTask(void)
{
	...
	while(w >= 12+27+5+32u)
	{
		...
		TCPPutString(MySocket, (const uint8_t*)": We are currently achieving ");
		TCPPutArray(MySocket, (const uint8_t*)"       ", 5-strlen((char*)vBytesPerSecond));
		TCPPutString(MySocket, vBytesPerSecond);
		TCPPutString(MySocket, (const uint8_t*)"00 bytes/second TX throughput.\r\n");
	...
	dw = SYS_TICK_Get() - dwTimeStart;
	if(dw > SYS_TICK_TicksPerSecondGet()/10)
	{
//		qw = ((uint64_t)dwBytesSent) * (SYS_TICK_TicksPerSecondGet()/100/8);
//		qw = ((uint64_t)dwBytesSent) * (SYS_TICK_TicksPerSecondGet()/100);
//		qw /= dw;
		qw = ((uint64_t)dwBytesSent)*(SYS_TICK_TicksPerSecondGet()/100) + (dw>>1);
		qw /= dw;
...
void TCPRXPerformanceTask(void)
{
	...
	dw = SYS_TICK_Get() - dwTimeStart;
	if(dw > SYS_TICK_TicksPerSecondGet())
	{
		...
		qw = (uint64_t)dwBytesRead * SYS_TICK_TicksPerSecondGet();
		qw += dw>>1;
		qw /= dw;
		ultoa((uint32_t)qw, vBuffer);
		TCPPutString(MySocket, vBuffer);
		TCPPutString(MySocket, (const uint8_t*)" bytes/second\r\n");
...
#endif //#if defined(TCPIP_STACK_USE_TCP_PERFORMANCE_TEST)
