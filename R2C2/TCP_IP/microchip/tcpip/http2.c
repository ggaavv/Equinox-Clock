...
/****************************************************************************
  ...
  ***************************************************************************/
		...
		HTTP_CONN					HTTPControlBlocks[HTTP_MAX_CONNECTIONS];
	...
	#endif
	HTTP_STUB httpStubs[HTTP_MAX_CONNECTIONS];	// HTTP stubs with state machine and socket
...
// IfCount tells us how many interfaces are up
static uint16_t IfCount;
...
/*****************************************************************************
  ...
  ***************************************************************************/
bool HTTPInit(const TCPIP_STACK_MODULE_CTRL* const stackData,
              const HTTP_MODULE_GONFIG* const httpData)
{
    int fail;
    ...
    for(curHTTPID = 0; curHTTPID < HTTP_MAX_CONNECTIONS ; curHTTPID++)
    {
        smHTTP = SM_HTTP_IDLE;
        sktHTTP = TCPOpenServer(IP_ADDRESS_TYPE_IPV4, HTTP_PORT, 0);
        if( sktHTTP != INVALID_SOCKET)
        {   // opened socket
        ...
        #if !defined(HTTP_SAVE_CONTEXT_IN_PIC_RAM)		
            PTR_BASE oldPtr = MACSetWritePtr(BASE_HTTPB_ADDR + curHTTPID*sizeof(HTTP_CONN));
        ...
        {
            fail = 1;
            // continue so that all sockets are properly initialized
            // just in case the caller of HTTPInit() chooses
            // to go on without verifying the outcome
        }
    }
...
/*****************************************************************************
  ...
  Remarks:
	This function is called only one during lifetime of the application.
  ***************************************************************************/
void HTTPDeInit(const TCPIP_STACK_MODULE_CTRL* const stackData)
{
	uint8_t connLoop;
	
	for (connLoop = 0; connLoop < HTTP_MAX_CONNECTIONS; connLoop++)
	{
		// Close the connections that were associated with this interface
		if (httpStubs[connLoop].socket != INVALID_SOCKET)
		{
			if (TCPSocketGetNet(httpStubs[connLoop].socket) == stackData->pNetIf)
			{
				TCPClose(httpStubs[connLoop].socket);
				httpStubs[connLoop].socket = INVALID_SOCKET;
				HTTPControlBlocks[connLoop].file = MPFS_INVALID_HANDLE;
				HTTPControlBlocks[connLoop].offsets = MPFS_INVALID_HANDLE;
			}
		}
	}
...
void HTTPServer(void)
{
	...
	for(conn = 0; conn < HTTP_MAX_CONNECTIONS; conn++)
	{
    ...
    do
    {
        ...
        case SM_HTTP_IDLE:
            ...
            {// Clear out state info and move to next state
				...
				curHTTP.httpTick = SYS_TICK_Get() + HTTP_TIMEOUT*SYS_TICK_TicksPerSecondGet();
		...
		case SM_HTTP_PARSE_REQUEST:
			...
			{// First line isn't here yet
				...
				if(SYS_TICK_Get() > curHTTP.httpTick)
			...
			lenA = TCPFind(sktHTTP, ' ', 0, 0, false);
			if(lenA > 5u)
				lenA = 5;
			TCPGetArray(sktHTTP, curHTTP.data, lenA+1);

		    if ( memcmp(curHTTP.data, (const void*)"GET", 3) == 0)
			    curHTTP.httpStatus = HTTP_GET;
			#if defined(HTTP_USE_POST)
		    else if ( memcmp(curHTTP.data, (const void*)"POST", 4) == 0)
			...
			lenA = TCPFind(sktHTTP, ' ', 0, 0, false);
			lenB = TCPFind(sktHTTP, '?', 0, lenA, false);
			lenA = mMIN(lenA, lenB);
			...
			#if defined(HTTP_MPFS_UPLOAD)
			if(memcmp(&curHTTP.data[1], HTTP_MPFS_UPLOAD, sizeof(HTTP_MPFS_UPLOAD)) == 0)
			...
			if(curHTTP.file == MPFS_INVALID_HANDLE)
			{
				...
				if(TCPIsSSL(sktHTTP))
				{
					strcpy((void*)&curHTTP.data[lenB], HTTPS_DEFAULT_FILE);
					lenB += strlen(HTTPS_DEFAULT_FILE);
				}
				else
				#endif
				{
					strcpy((void*)&curHTTP.data[lenB], HTTP_DEFAULT_FILE);
			...
			for(curHTTP.fileType = HTTP_TXT; curHTTP.fileType < HTTP_UNKNOWN; curHTTP.fileType++)
				if(!stricmppgm2ram(ext, (const void*)httpFileExtensions[curHTTP.fileType]))
					break;
		...
		case SM_HTTP_PARSE_HEADERS:
			...
			while(1)
			{
				...
				lenB = TCPFind(sktHTTP, ':', 0, lenA, false) + 2;
				isDone = false;
		...
		case SM_HTTP_SERVE_HEADERS:
			...
			// Send headers
			TCPPutString(sktHTTP, (const uint8_t*)HTTPResponseHeaders[curHTTP.httpStatus]);
			...
			if(curHTTP.httpStatus == HTTP_REDIRECT)
			{
				...
				TCPPutString(sktHTTP, (const uint8_t*)"\r\n\r\n304 Redirect: ");
				TCPPutString(sktHTTP, curHTTP.data);
				TCPPutString(sktHTTP, (const uint8_t*)HTTP_CRLF);
			}
			...
			{
				TCPPutString(sktHTTP, (const uint8_t*)"Content-Type: ");
				TCPPutString(sktHTTP, (const uint8_t*)httpContentTypes[curHTTP.fileType]);
				TCPPutString(sktHTTP, HTTP_CRLF);
			}
			
			// Output the gzip encoding header if needed
			if(MPFSGetFlags(curHTTP.file) & MPFS2_FLAG_ISZIPPED)
			{
				TCPPutString(sktHTTP, (const uint8_t*)"Content-Encoding: gzip\r\n");
			}
						
			// Output the cache-control
			TCPPutString(sktHTTP, (const uint8_t*)"Cache-Control: ");
			if(curHTTP.httpStatus == HTTP_POST || curHTTP.nextCallback != 0xffffffff)
			{// This is a dynamic page or a POST request, so no cache
				TCPPutString(sktHTTP, (const uint8_t*)"no-cache");
			}
			else
			{// This is a static page, so save it for the specified amount of time
				TCPPutString(sktHTTP, (const uint8_t*)"max-age=");
				TCPPutString(sktHTTP, (const uint8_t*)HTTP_CACHE_LEN);
			}
			TCPPutString(sktHTTP, HTTP_CRLF);
		...
		case SM_HTTP_SERVE_COOKIES:
			...
			{
				// Write the header
				TCPPutString(sktHTTP, (const uint8_t*)"Set-Cookie: ");
...
static void HTTPHeaderParseAuthorization(void)
{
	...
	len = TCPFindArray(sktHTTP, HTTP_CRLF, HTTP_CRLF_LEN, 0, 0, false);
	len += 3;
	len &= 0xfc;
...
static void HTTPHeaderParseCookie(void)
{
	...
	// Verify there's enough space
	lenB = TCPFindArray(sktHTTP, HTTP_CRLF, HTTP_CRLF_LEN, 0, 0, false);
	...
	while(lenB != 0u)
	{
		...
		*(curHTTP.ptrData++) = '&';
		...
		{
			TCPGet(sktHTTP, NULL);
			while(TCPFind(sktHTTP, ' ', 0, 0, false) == 0u)
				TCPGet(sktHTTP, NULL);
		}
...
static HTTP_READ_STATUS HTTPReadTo(uint8_t cDelim, uint8_t* cData, uint16_t wLen)
{
	...
	if(cDelim)
		wPos = TCPFind(sktHTTP, cDelim, 0, 0, false);
	else
...
/*****************************************************************************
  Function:
	void HTTPIncFile(const uint8_t* cFile)
...
MPFS_HANDLE	  HTTPCurConnectionFileGet(void)
{
	return curHTTP.file;
}
uint8_t		  HTTPCurConnectionPostSmGet(void)
{
	return curHTTP.smPost;
}
void		  HTTPCurConnectionPostSmSet(uint8_t stat)
{
	curHTTP.smPost = stat;
}
uint8_t*          HTTPCurConnectionDataBufferGet(void)
{
	return curHTTP.data;
}
uint32_t          HTTPCurConnectionCallbackPosGet(void)
{
	return curHTTP.callbackPos;
}
void	          HTTPCurConnectionCallbackPosSet(uint32_t pos)
{
	curHTTP.callbackPos = pos;
}
void	          HTTPCurConnectionStatusSet(HTTP_STATUS stat)
{
	curHTTP.httpStatus = stat;
}
void	          HTTPCurConnectionHasArgsSet(bool args)
{
	curHTTP.hasArgs = args;
}
uint32_t          HTTPCurConnectionByteCountGet(void)
{
	return curHTTP.byteCount;
}
void          	  HTTPCurConnectionByteCountSet(uint32_t byteCount)
{
	curHTTP.byteCount = byteCount;
}
void          	  HTTPCurConnectionByteCountDec(uint32_t byteCount)
{
	curHTTP.byteCount -= byteCount;
}
TCP_SOCKET        HTTPCurConnectionSocketGet(void)
{
	return httpStubs[curHTTPID].socket;
}
