...
/*****************************************************************************
  Function:
	void TFTPUploadRAMFileToHost(const uint8_t *vRemoteHost, 
  ...
  ***************************************************************************/
void TFTPUploadRAMFileToHost(const uint8_t *vRemoteHost, const uint8_t *vFilename, uint8_t *vData, uint16_t wDataLength)
{
...
/*****************************************************************************
  Function:
	void TFTPUploadFragmentedRAMFileToHost(const uint8_t *vRemoteHost, 
										   const uint8_t *vFilename, 
  ...
  Remarks:
	...
	TCPIP_STACK_USE_DNS must be defined in tcpip_config.h.
  	...
  	any other TFTP API functions.
  ***************************************************************************/
void TFTPUploadFragmentedRAMFileToHost(const uint8_t *vRemoteHost, const uint8_t *vFilename, TFTP_CHUNK_DESCRIPTOR *vFirstChunkDescriptor)
{
...
int8_t TFTPGetUploadStatus(void)
{
	...
	if(UDPIsOpened(_tftpSocket)== false)
	{
		...
		_tftpSocket = UDPOpen((uint32_t)(ROM_PTR_BASE)vUploadRemoteHost,
	...
	case TFTP_UPLOAD_GET_DNS:
        if(DNSBeginUsage(0) != DNS_RES_OK)
			break;
		DNSResolve(vUploadRemoteHost, DNS_TYPE_A);
		smUpload = TFTP_UPLOAD_RESOLVE_HOST;
		break;

	case TFTP_UPLOAD_RESOLVE_HOST:
        DNS_RESULT dnsRes = DNSIsResolved(vUploadRemoteHost, &ipRemote);
