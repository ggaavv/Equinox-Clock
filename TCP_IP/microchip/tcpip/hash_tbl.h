...
struct _TAG_OA_HASH_DCPT 
{
    ...
    size_t                  keyLen;     // the length of the key field for this hash 
#endif  // defined ( OA_HASH_DYNAMIC_KEY_MANIPULATION )

    // fields updated by the OAHashInit()
...
// function that copies a key to a destination entry
void            OAHashKeyCopy(OA_HASH_DCPT* pOH, OA_HASH_ENTRY* dstEntry, void* srcKey);
