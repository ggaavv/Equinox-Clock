...
/***************************************************************
  ...
  Summary:
	Writes an Host-Name to the display as available 
...
MDNS_STATIC void DisplayHostName(uint8_t *HostName)
{
	...
	SYS_CONSOLE_MESSAGE("\r\nZeroConf: Host = ");
	SYS_CONSOLE_MESSAGE(HostName);
	SYS_CONSOLE_MESSAGE("\r\n");

	SYS_OUT_MESSAGE(HostName);
}
...
MDNS_STATIC void mDNSPutString(uint8_t* string)
{
	...
	while(1)
	{
        ...
        UDPPut(mDNS_socket, len);
        UDPPutArray(mDNS_socket, label, len);
		string =  right_ptr;
...
MDNS_STATIC bool mDNSProbe(mDNSProcessCtx_common *pCtx)
{
	...
	/*
    UDPPut(mDNS_socket, gResponderCtx.query_id.v[1]);// User chosen transaction ID
    UDPPut(mDNS_socket, gResponderCtx.query_id.v[0]);

    UDPPut(mDNS_socket, 0x00);		// Standard query with recursion
	UDPPut(mDNS_socket, 0x00);	
	
	UDPPut(mDNS_socket, 0x00);		// 1 entry in the question section
	UDPPut(mDNS_socket, 0x01);
	
	UDPPut(mDNS_socket, 0x00);		// 0 entry in the answer section
	UDPPut(mDNS_socket, 0x00);
	
	UDPPut(mDNS_socket, 0x00);		// 1 entry in name server section
	UDPPut(mDNS_socket, 0x01);		

	UDPPut(mDNS_socket, 0x00);		// 0 entry in additional records section
	UDPPut(mDNS_socket, 0x00);
	*/
	...
	UDPPutArray(mDNS_socket, (uint8_t *) &mDNS_header, sizeof(MDNS_MSG_HEADER));
	...
	UDPPut(mDNS_socket, 0x00);			// Type: Always QTYPE_ANY
	UDPPut(mDNS_socket, QTYPE_ANY);

	UDPPut(mDNS_socket, 0x80);			// Class: Cache-Flush
	UDPPut(mDNS_socket, 0x01);			//        IN (Internet)
	...
	UDPPut(mDNS_socket, 0x00);		// Type: A or SRV 

	switch (pCtx->type)
	{
	case MDNS_CTX_TYPE_HOST:
		UDPPut(mDNS_socket, QTYPE_A);
		break;

	case MDNS_CTX_TYPE_SD:
		UDPPut(mDNS_socket, QTYPE_SRV);
		break;
	}

		

	UDPPut(mDNS_socket, 0x00);		// Class: Cache-Flush bit MUST NOT be set
	UDPPut(mDNS_socket, 0x01);		//        IN (Internet)

	UDPPut(mDNS_socket, 0x00);		// 0x00000078 Time To Live, 2 minutes
	UDPPut(mDNS_socket, 0x00);
	UDPPut(mDNS_socket, 0x00);
	UDPPut(mDNS_socket, 0x78);

	switch (pCtx->type)
	{
	case MDNS_CTX_TYPE_HOST:
		{
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].rdlength.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].rdlength.v[0]);

			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].ip.v[0]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].ip.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].ip.v[2]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_A_INDEX].ip.v[3]);
	...
	case MDNS_CTX_TYPE_SD:
		{
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].rdlength.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].rdlength.v[0]);

			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.priority.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.priority.v[0]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.weight.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.weight.v[0]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.port.v[1]);
			UDPPut(mDNS_socket, gResponderCtx.rr_list[QTYPE_SRV_INDEX].srv.port.v[0]);
...
//rex	}
	...
	UDPPut(mDNS_socket, 0x00); UDPPut(mDNS_socket, record_type);		// Resource Record Type
     ...
     * information */
	if (UDPSocketDcpt[mDNS_socket].remotePort == MDNS_PORT)
	{
		UDPPut(mDNS_socket, cFlush); UDPPut(mDNS_socket, 0x01);		// Class 
	}
	else
	{
		// Legacy/Unicast DNS response should not set the Cache-Flush bit.
		UDPPut(mDNS_socket, 0x00); UDPPut(mDNS_socket, 0x01);			// Class 
	}

    UDPPut(mDNS_socket, ttl.v[3]);		// Time To Live
    UDPPut(mDNS_socket, ttl.v[2]);
    UDPPut(mDNS_socket, ttl.v[1]);
    UDPPut(mDNS_socket, ttl.v[0]);

	switch (record_type)
	{
	case QTYPE_A:
    
        UDPPut(mDNS_socket, 0x00);	// 0x0004 Data length
        UDPPut(mDNS_socket, 0x04);	
        UDPPut(mDNS_socket, pRecord->ip.v[0]);	// Put out IP address
        UDPPut(mDNS_socket, pRecord->ip.v[1]);	// NetConfig.MyIPAddr.v[1]
        UDPPut(mDNS_socket, pRecord->ip.v[2]);
        UDPPut(mDNS_socket, pRecord->ip.v[3]);
	...
	case QTYPE_PTR:
        ...
        pRecord->rdlength.Val = strlen(pRecord->rdata) + 2 ;

        UDPPut(mDNS_socket, pRecord->rdlength.v[1]);
        UDPPut(mDNS_socket, pRecord->rdlength.v[0]); // Res-Data Length
	...
	case QTYPE_SRV:
        ...
        UDPPut(mDNS_socket, pRecord->srv.priority.v[1]);	// Put Priority
        UDPPut(mDNS_socket, pRecord->srv.priority.v[0]);
        UDPPut(mDNS_socket, pRecord->srv.weight.v[1]);
        UDPPut(mDNS_socket, pRecord->srv.weight.v[0]);
        UDPPut(mDNS_socket, pRecord->srv.port.v[1]);
        UDPPut(mDNS_socket, pRecord->srv.port.v[0]);        
	...
	case QTYPE_TXT:
        ...
        rec_length = strlen(pRecord->rdata);
        pRecord->rdlength.Val = rec_length + 1;

        UDPPut(mDNS_socket, pRecord->rdlength.v[1]);
        UDPPut(mDNS_socket, pRecord->rdlength.v[0]); // Res-Data Length

        UDPPut(mDNS_socket, pRecord->rdlength.Val-1); // As of now only single TXT string supported!!

        UDPPutArray(mDNS_socket, pRecord->rdata,rec_length);
...
MDNS_STATIC size_t mDNSSDFormatServiceInstance(uint8_t *string, size_t strSize )
{
	...
	*temp++ = '\0';
	return strncpy_m(string, strSize, 1, output);
}		
...
void mDNSSDFillResRecords(mDNSProcessCtx_sd *sd)
{
    ...
    srv_name_len = strlen(sd->srv_name);
    srv_type_len = strlen(sd->srv_type); 
    ...
    strncpy(sd->sd_qualified_name, sd->srv_name, sizeof(sd->sd_qualified_name));
    qual_len= mDNSSDFormatServiceInstance(sd->sd_qualified_name, sizeof(sd->sd_qualified_name));

    strncpy_m(sd->sd_qualified_name + qual_len, sizeof(sd->sd_qualified_name), 2, ".", sd->srv_type);
    ...
    strncpy_m(rr_list->rdata + srv_name_len, sizeof(sd->sd_qualified_name) - srv_name_len, 2, ".", sd->srv_type);
    ...
    rr_list->rdlength.Val = strlen(rr_list->rdata)+2+6;
				  ...
				  const uint8_t	*txt_record)
{
    ...
    if( sd->used)
    {
        ...
        if(txt_record != NULL)
        {
			sd->sd_txt_rec_len = strncpy_m(sd->sd_txt_rec, sizeof(sd->sd_txt_rec), 1, (uint8_t *) txt_record );
					...
					void *context)
{
    ...
    strncpy(gSDCtx.srv_name, (uint8_t *)srv_name, sizeof(gSDCtx.srv_name));
    
    strncpy(gSDCtx.srv_type, (uint8_t *)srv_type, sizeof(gSDCtx.srv_type));
    gSDCtx.sd_call_back = call_back;
    gSDCtx.sd_context = context;

	gSDCtx.sd_txt_rec_len = strncpy_m(gSDCtx.sd_txt_rec, sizeof(gSDCtx.sd_txt_rec), 1, (uint8_t *) txt_record);
...
MDNS_STATIC uint16_t mDNSFetch(uint16_t wOffset, uint16_t wLen, uint8_t *pcString)
{
	...
	UDPSetRxOffset(mDNS_socket, wOffset);

	rc = UDPGetArray(mDNS_socket, pcString, wLen);
...
MDNS_STATIC uint16_t mDNSDeCompress(uint16_t wPos, uint8_t *pcString, bool bFollowPtr, uint8_t cElement, uint8_t cDepth)
{
	...
	{
		rr_len++;
		if(!UDPGet(mDNS_socket, &substr_len))
		...
		if((substr_len & 0xC0) == 0xC0)	// b'11 at MSb indicates compression ptr
		{
			...
			if (bFollowPtr)
			{
				...
				UDPSetRxOffset(mDNS_socket, offset_in_ptr);
		...
		else
		{
			if (pcString)
			{
				...
				UDPGetArray(mDNS_socket, pcString, substr_len);
				pcString += substr_len;
			}
			else
			{
				...
				while (i--)
				{
					UDPGet(mDNS_socket, mDNS_socket, &tmp);
				}
			}
...
mDNSDeCompress_done:
	...
	UDPSetRxOffset(mDNS_socket, endOffset);
					  ...
					  uint16_t				idxRR)
{
	...
	// TYPE & CLASS
	UDPGet(mDNS_socket, &res_rec.type.v[1]);	
	UDPGet(mDNS_socket, &res_rec.type.v[0]);
	UDPGet(mDNS_socket, &res_rec.class.v[1]);
	UDPGet(mDNS_socket, &res_rec.class.v[0]);
	g_mDNS_offset += 4;
	...
	UDPGet(mDNS_socket, &res_rec.ttl.v[3]);		// Time to live
	UDPGet(mDNS_socket, &res_rec.ttl.v[2]);
	UDPGet(mDNS_socket, &res_rec.ttl.v[1]);
	UDPGet(mDNS_socket, &res_rec.ttl.v[0]);
	UDPGet(mDNS_socket, &res_rec.rdlength.v[1]);		// Response length
	UDPGet(mDNS_socket, &res_rec.rdlength.v[0]);
	g_mDNS_offset += 6;
	...
	{
	case QTYPE_A:
		UDPGet(mDNS_socket, &res_rec.ip.v[0]); // Read out IP address
		UDPGet(mDNS_socket, &res_rec.ip.v[1]);
		UDPGet(mDNS_socket, &res_rec.ip.v[2]);
		UDPGet(mDNS_socket, &res_rec.ip.v[3]);
	...
	case QTYPE_SRV:           
		...
		UDPGet(mDNS_socket, &res_rec.srv.priority.v[1]); // Put Priority, weight, port
		UDPGet(mDNS_socket, &res_rec.srv.priority.v[0]);
		UDPGet(mDNS_socket, &res_rec.srv.weight.v[1]);
		UDPGet(mDNS_socket, &res_rec.srv.weight.v[0]);
		UDPGet(mDNS_socket, &res_rec.srv.port.v[1]);
		UDPGet(mDNS_socket, &res_rec.srv.port.v[0]);
...
MDNS_STATIC void mDNSResponder(void)
{
	...
	switch(mDNS_responder_state)
	{
		case MDNS_RESPONDER_INIT:
			...
			mDNS_socket = UDPOpen((uint32_t)&mDNSRemote,UDP_OPEN_NODE_INFO,MDNS_PORT,MDNS_PORT);
		...
		case MDNS_RESPONDER_LISTEN:
			...
			else
			{
				...
				memcpy((void*)&UDPSocketDcpt[mDNS_socket].remoteNode, 
						(const void*)&mDNSRemote, sizeof(mDNSRemote));
				UDPSocketDcpt[mDNS_socket].remotePort = MDNS_PORT;
				UDPSocketDcpt[mDNS_socket].localPort = MDNS_PORT;
			}
...
void mDNSFillHostRecord(void)
{
	...
	for (i=0; i<=3; i++)
		gResponderCtx.rr_list[QTYPE_A_INDEX].ip.v[i] = NetConfig.MyIPAddr.v[i];
...
MDNSD_ERR_CODE mDNSHostRegister(const char *host_name)
{
    ...
    strncpy(gHostCtx.szUserChosenHostName, host_name, sizeof(gHostCtx.szUserChosenHostName));
    strncpy_m(gHostCtx.szHostName, sizeof(gHostCtx.szHostName), 3, gHostCtx.szUserChosenHostName, ".", CONST_STR_local);
...
void mDNSInitialize(const char *szHostName)
{
    ...
    gResponderCtx.prev_ipaddr.Val = NetConfig.MyIPAddr.Val;
				...
				else
				{
					...
					if (pCtx->nClaimCount >= MDNS_ANNOUNCE_NUM) 
					{
						...
						if ( bIsHost )
						{
							...
							// NetConfig.NetBIOSName, so update it here.
							strncpy(NetConfig.NetBIOSName, gHostCtx.szHostName, sizeof(NetConfig.NetBIOSName));
						...
						{
							SYS_CONSOLE_MESSAGE("\r\nZeroConf: Service = ");
							SYS_CONSOLE_MESSAGE(gSDCtx.sd_qualified_name);
							SYS_CONSOLE_MESSAGE("\r\n");
...
void mDNSProcess(void)
{
    ...
    if(NetConfig.MyIPAddr.Val == 0x00)
    {
        return;
    }
    else if (NetConfig.MyIPAddr.Val != gResponderCtx.prev_ipaddr.Val) {
...
mDNSMulticastFilterRegister(void)
{
	...
	#if /* PIC32MX6XX/7XX Internal Ethernet controller */ (defined(__PIC32MX__) && defined(_ETH) && !defined(ENC100_INTERFACE_MODE) && !defined(ENC_CS_TRIS) && !defined(TCPIP_IF_MRF24W)) || \
		/* ENC424J600/624J600 */						  defined(ENC100_INTERFACE_MODE) || \
		/* ENC28J60 */									  defined(ENC_CS_TRIS)
		SetRXHashTableEntry(*((MAC_ADDR*)mcast_addr));
	#elif /* MRF24W */ defined(TCPIP_IF_MRF24W)
...
void mDNSDumpInfo(void)
{
	...
	SYS_CONSOLE_MESSAGE("   Host registered: "); SYS_CONSOLE_MESSAGE(gHostCtx.szUserChosenHostName); SYS_CONSOLE_MESSAGE("\r\n");
	SYS_CONSOLE_MESSAGE("         qualified: "); SYS_CONSOLE_MESSAGE(gHostCtx.szHostName); SYS_CONSOLE_MESSAGE("\r\n");
	SYS_CONSOLE_MESSAGE("Service registered: "); SYS_CONSOLE_MESSAGE(gSDCtx.srv_name); SYS_CONSOLE_MESSAGE("\r\n");
	SYS_CONSOLE_MESSAGE("         qualified: "); SYS_CONSOLE_MESSAGE(gSDCtx.sd_qualified_name); SYS_CONSOLE_MESSAGE("\r\n");
	sprintf((char *) tmp, "%d", gSDCtx.sd_port);
	SYS_CONSOLE_MESSAGE("              port: "); SYS_CONSOLE_MESSAGE(tmp); SYS_CONSOLE_MESSAGE("\r\n");
	SYS_CONSOLE_MESSAGE("    TXT registered: "); SYS_CONSOLE_MESSAGE(gSDCtx.sd_txt_rec); SYS_CONSOLE_MESSAGE("\r\n");
}

//#endif

#endif //#if defined(TCPIP_STACK_USE_ZEROCONF_MDNS_SD)
