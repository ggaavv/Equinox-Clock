/*
 * This file was automatically generated on Wed Apr 28 19:41:06 2010
 * by mib2bib utility.

 * This file contains 'C' defines for dynamic OIDs and AgentID only.
 * Do not modify this file manually.
 * Include this file in your application source file that handles SNMP callbacks and TRAP.
 */
#define MICROCHIP (255)		// This is an Agent ID for use in SNMPNotify() only.
#define SYS_UP_TIME (250)			// 43.6.1.2.1.1.3: READONLY TIME_TICKS.
#define TRAP_RECEIVER_ID (1)			// 43.6.1.4.1.17095.2.1.1.1: READWRITE uint8_t.
#define TRAP_RECEIVER_ENABLED (2)			// 43.6.1.4.1.17095.2.1.1.2: READWRITE uint8_t.
#define TRAP_RECEIVER_IP (3)			// 43.6.1.4.1.17095.2.1.1.3: READWRITE IP_ADDRESS.
#define TRAP_COMMUNITY (4)			// 43.6.1.4.1.17095.2.1.1.4: READWRITE ASCII_STRING.
#define LED_D5 (5)			// 43.6.1.4.1.17095.3.1: READWRITE uint8_t.
#define LED_D6 (6)			// 43.6.1.4.1.17095.3.2: READWRITE uint8_t.
#define PUSH_BUTTON (7)			// 43.6.1.4.1.17095.3.3: READONLY uint8_t.
#define ANALOG_POT0 (8)			// 43.6.1.4.1.17095.3.4: READONLY uint16_t.
#define LCD_DISPLAY (10)			// 43.6.1.4.1.17095.3.6: READWRITE ASCII_STRING.
