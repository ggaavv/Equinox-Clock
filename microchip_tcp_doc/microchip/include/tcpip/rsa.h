...
bool RSAInit(const TCPIP_STACK_MODULE_CTRL* const stackData,
             const RSA_MODULE_GONFIG* const rsaData);
void RSADeInit(const TCPIP_STACK_MODULE_CTRL* const stackData);
bool RSABeginUsage(NET_CONFIG* pIf, RSA_OP op, uint16_t vKeyByteLen);
void RSAEndUsage(NET_CONFIG* pIf);
void RSASetData(NET_CONFIG* pIf, uint8_t* data, uint16_t len, RSA_DATA_FORMAT format);
void RSASetResult(uint8_t* data, RSA_DATA_FORMAT format);
RSA_STATUS RSAStep(NET_CONFIG* pIf);

#define RSABeginEncrypt(pIf, a)	RSABeginUsage(pIf, RSA_OP_ENCRYPT, a)
#define RSAEndEncrypt(pIf)		RSAEndUsage(pIf)
void RSASetE(uint8_t* data, uint8_t len, RSA_DATA_FORMAT format);
void RSASetN(uint8_t* data, RSA_DATA_FORMAT format);

#define RSABeginDecrypt(pIf)	RSABeginUsage(pIf, RSA_OP_DECRYPT, SSL_RSA_KEY_SIZE/8)
#define RSAEndDecrypt(pIf)		RSAEndUsage(pIf)
