...
static const char * const FTPCommandString[] =
{
    "USER",                         // FTP_CMD_USER
    "PASS",                         // FTP_CMD_PASS
    "QUIT",                         // FTP_CMD_QUIT
...
static const char * const FTPResponseString[] =
{
    "220 Ready\r\n",                    // FTP_RESP_BANNER
...
static SYS_TICK         lastActivity;   // Timeout keeper.
...
bool FTPServer(void)
{
    ...
    else if ( smFTP != SM_FTP_NOT_CONNECTED )
    {
        ...
        if ( currentTick >= (FTP_TIMEOUT * SYS_TICK_TicksPerSecondGet()) )
        {
            lastActivity                = SYS_TICK_Get();
...
SM_FTP_RESPOND_Label:
        ...
        if(TCPIsPutReady(FTPSocket) < strlen(FTPResponseString[FTPResponse]))
			return true;

		TCPPutString(FTPSocket, (const uint8_t*)FTPResponseString[FTPResponse]);
		TCPFlush(FTPSocket);
...
static bool PutFile(void)
{
    ...
    case SM_FTP_CMD_IDLE:
        ...
        else
        {
            ...
            FTPDataSocket   = TCPOpen((PTR_BASE)&TCPGetSocketInfo(FTPSocket)->remote, TCP_OPEN_NODE_INFO, FTPDataPort.Val, TCP_PURPOSE_FTP_DATA);
...
static FTP_COMMAND ParseFTPCommand(uint8_t *cmd)
{
	...
	{
		if ( !memcmp((void*)cmd, (const void*)FTPCommandString[i], 3) )
			return i;
	}
