    ...
    if ( g_encIndex[encPtrId] < BASE_TCB_ADDR )
    {
        ...
        else
        {
            ...
            if ( !RawWindowReady[RAW_TX_ID] )
            {
                ...
                maxAllowedTicks = SYS_TICK_TicksPerSecondGet() * 6;  /* 2 second timeout, needed if data traffic and scanning occurring */
        ...
        SYS_ASSERT(RawWindowReady[rawId], "");
...
#if defined(TCPIP_STACK_CLIENT_MODE) && defined(USE_GRATUITOUS_ARP)
 ...
 *****************************************************************************/
void WFPeriodicGratuitousArp(TCPIP_MAC_HANDLE hMac)
{
	static SYS_TICK oldTime = 0, currTime;

    if (!MRF24W_MACIsLinked())
    {
        return;
    }    

	currTime = SYS_TICK_Get();
	
	if ( (currTime < oldTime) //wrap-around case
			||
		 ((currTime - oldTime) > WFArpBroadcastIntervalSec*SYS_TICK_TicksPerSecondGet())
		)
	{
        NET_CONFIG* pConfig = _TCPIPStackMacToNet(hMac);
        ARPResolve(&(pConfig->MyIPAddr));	
        oldTime = currTime;
	}
}
#endif //defined(TCPIP_STACK_CLIENT_MODE) && defined(USE_GRATUITOUS_ARP)
...
uint16_t WFGetTCBSize(void)
{
 ...
 /******************************************************************************
 * Function:        void MRF24W_MACInit(NET_CONFIG* pNetIf)
 *
 * PreCondition:    None
 *
...
/******************************************************************************
 * Function:        bool MRF24W_MACIsLinked(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 *                        call to MRF24W_MACIsLinked()
 *                  false: If the PHY reports no link partner, or the link went
 *                         down momentarily since the last call to MRF24W_MACIsLinked()
 *
 * Side Effects:    None
 *
...
bool WFisTxMgmtReady(void)
{
    ...
    else
    {
        ...
        /* See comment in MRF24W_MACProcess */
...
/******************************************************************************
 * Function:        WORD MRF24W_MACGetFreeRxSize(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * PARAMS:  None
 *
 *  NOTES: Called by MRF24W_MACGetHeader() to see if any data packets have been received.
...
static uint16_t MACIFService(void)
{
    ...
    SYS_ASSERT(byteCount > 0, "");         /* byte count should never be 0 */
 ...
 *
 * PreCondition:    A packet has been created by calling MRF24W_MACPut() and
 *                  MRF24W_MACPutHeader().
 *
 * Input:           None
 *
 ...
 *                  MRF24W_MACFlush() again.  Until MRF24W_MACPutHeader() or MRF24W_MACPut() is
...
void MRF24W_MACFlush(void)
{
    ...
    SYS_ASSERT(RawWindowReady[RAW_TX_ID], "");
 ...
 *                  MRF24W_MACPut() and MRF24W_MACPutArray() will use this new value.
 *
 * Note:            None
 *****************************************************************************/
...
uint16_t MRF24W_MACCalcRxChecksum(TCPIP_MAC_HANDLE hMac, uint16_t offset, uint16_t len)
{
    ...
    temp = CalcMACBufferIPChecksum(hMac, len);
...
uint16_t MRF24W_MACCalcIPBufferChecksum(TCPIP_MAC_HANDLE hMac, uint16_t len)
{
    ...
    return CalcMACBufferIPChecksum(hMac, len);
}

/******************************************************************************
 * Function:        void MRF24W_MACMemCopyAsync(uint16_t destAddr, uint16_t sourceAddr, uint16_t len)
 *
 ...
 *                  MRF24W_MACIsMemCopyDone() to see when the transfer is complete.
 *
...
void MRF24W_MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, uint16_t len)
{
    ...
    if ( (len > 0u) && (destAddr >= BASE_TCB_ADDR) && (sourceAddr >= BASE_TCB_ADDR) )
    {
        ...
        {
            SYS_ASSERT(false, "");  /* this should never happen (that can't mount scratch on either RAW window) */
        }         
...
/******************************************************************************
 * Function:        uint8_t MRF24W_MACGet()
 *
 * PreCondition:    SPI bus must be initialized (done in MRF24W_MACInit()).
 ...
 * Input:           None
 *
 * Output:          uint8_t read from the MRF24W's RAM
 *
 * Side Effects:    None
 *
 * Overview:        MRF24W_MACGet returns the byte pointed to by ERDPT and
 *                  increments ERDPT so MRF24W_MACGet() can be called again.  The
 ...
 *                  it performs much faster than multiple MRF24W_MACGet() calls.
 *                  ERDPT is incremented after each byte, following the same
 *                  rules as MRF24W_MACGet().
 *
 * Note:            None
 *****************************************************************************/
...
/******************************************************************************
 * Function:        void MRF24W_MACPut(uint8_t val)
 *
 * PreCondition:    SPI bus must be initialized (done in MRF24W_MACInit()).
 *                  EWRPT must point to the location to begin writing.
 *
 * Input:           uint8_t to write into the MRF24W buffer memory
 *
 * Output:          None
 *
 * Side Effects:    None
 *
 * Overview:        MRF24W_MACPut outputs the Write Buffer Memory opcode/constant
 ...
 *                  MRF24W RAM.  It performs faster than multiple MRF24W_MACPut()
 ...
 *                  transmitted will most likely be lost.  MRF24W_MACIsLinked() can
...
/******************************************************************************
 * Function:        bool MRF24W_MACRxFilter(TCPIP_MAC_HANDLE hMac, NODE_INFO* pPkt)
 *
 * PreCondition:    None
 *
 * Input:           hMac - interface instance
...
bool MRF24W_MACRxFilter(TCPIP_MAC_HANDLE hMac, NODE_INFO* pPkt)
{
    ...
    if(memcmp((void*)&pPkt->MACAddr, (void*)&pConfig->MyMACAddr, 6) == 0u)
    {
