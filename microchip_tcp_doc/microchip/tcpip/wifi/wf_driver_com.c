...
{
    NET_CONFIG *p_config = GetNetworkConfig();
        
    //----------------------------------------------------------------------
    // if application wants PS-Poll mode and not waiting for a mgmt response
    //----------------------------------------------------------------------
    if ( (GetAppPowerSaveMode() == true) && (!g_WaitingForMgmtResponse) )
    {
        // if connection state changed (just connected or disconneted)
        if (g_WiFiConnectionChanged == true)
        {
            ...
            else
            {
                ...
                {
                    WF_PsPollEnable(g_rxDtim, g_aggressivePs);
        ...
        else if ((p_config->Flags.bIsDHCPEnabled == true) && (g_DhcpSuccessful == true))
        {
