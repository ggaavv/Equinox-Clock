...
/*static __inline__*/static  void /*__attribute__((always_inline))*/ _OAHashRemoveEntry(OA_HASH_DCPT* pOH, OA_HASH_ENTRY* pOE)
{
    pOE->flags.busy = 0;
    pOH->fullSlots--;
}
...
#else
    probeStep = OAHashProbeHash(pOH, key);
...
void OAHashInit(OA_HASH_DCPT* pOH)
{
    OA_HASH_ENTRY*  pHE;    
    size_t  ix;
...
OA_HASH_ENTRY* OAHashLookUp(OA_HASH_DCPT* pOH, void* key)
{
    OA_HASH_ENTRY*  pBkt;
    size_t      bkts = 0;
    size_t      bktIx;
...
#else
        if(pBkt->flags.busy && OAHashKeyCompare(pOH, pBkt, key) == 0)
        {   // found entry
            pBkt->flags.newEntry = 0;
            return pBkt;
        }
...
// fullSlots < hElements)
OA_HASH_ENTRY*   OAHashLookUpInsert(OA_HASH_DCPT* pOH, void* key)
{
...
#else
        if((pDel = OAHashDeleteEntry(pOH)) == 0)
        {   // nothing else we can do
            return 0;
        }
#endif  // defined ( OA_HASH_DYNAMIC_KEY_MANIPULATION )
...
// SHOULD be called when entries have to be deleted!
// because the hash also has to do some internal clean-up.
void OAHashRemoveAll(OA_HASH_DCPT* pOH)
{
    OA_HASH_ENTRY*  pBkt;
    size_t      bktIx;
...
OA_HASH_ENTRY* OAHashGetEntry(OA_HASH_DCPT* pOH, size_t entryIx)
{
...
#else
            OAHashKeyCopy(pOH, pBkt, key);   // set the key
#endif  // defined ( OA_HASH_DYNAMIC_KEY_MANIPULATION )
...
#else
        if(OAHashKeyCompare(pOH, pBkt, key) == 0)
        {   // found entry
            return pBkt;
        }
#endif  // defined ( OA_HASH_DYNAMIC_KEY_MANIPULATION )
