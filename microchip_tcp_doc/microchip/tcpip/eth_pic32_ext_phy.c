...
// Compile only for PIC32MX that has Ethernet MAC interface and TCPIP_IF_PIC32INT defined
...
#include "tcpip/eth_pic32_ext_phy_regs.h"
...
/*static __inline__*/static  eEthLinkStat __attribute__((unused/*always_inline*/)) _Phy2LinkStat(__BMSTATbits_t phyStat)
{
...
/*static __inline__*/static  unsigned short __attribute__((unused/*always_inline*/)) _PhyReadReg( unsigned int rIx, unsigned int phyAdd )
{
...
eEthRes __attribute__((weak)) EthPhyInit(eEthOpenFlags oFlags, eEthPhyCfgFlags cFlags, eEthOpenFlags* pResFlags)
{
	...
	EthMIIMConfig(SYS_CLK_ClockGet(), EthPhyMIIMClock());
...
eEthRes  __attribute__((weak)) EthPhyNegotiationComplete(int waitComplete)
{
	...
	{	// just protect from an accidental call
		...
		if(waitComplete)
		{
			...
			{	// not started yet
				tWait=(SYS_CLK_ClockGet()/2000)*PHY_NEG_INIT_TMO;
			...
			if(!phyBMCon.AN_RESTART)
			{	// ok, started
				tWait=(SYS_CLK_ClockGet()/2000)*PHY_NEG_DONE_TMO;
				tStart=ReadCoreTimer();
				do
				{
					phyStat.w=_PhyReadReg(PHY_REG_BMSTAT, _PhyAdd);
...
int  __attribute__((weak)) EthPhyReset(int waitComplete)
{
	...
	{	// wait reset self clear
		...
		tWaitReset=(SYS_CLK_ClockGet()/2000)*PHY_RESET_CLR_TMO;
		tStart=ReadCoreTimer();
		do
		{
